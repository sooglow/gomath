...
type Props = { onNavigate: (tab: TabType) => void };
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-20">
        ...
        <div className="space-y-1">
          {[
            ...
            <div key={item.label} className="flex items-center justify-between py-2.5">
              ...
              >
                <span className="absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all" style={{ left: item.value ? '22px' : '2px' }} />
              </button>
            </div>
          ))}
        </div>
      ...
      <button
        ...
        className="w-full py-3 text-sm font-bold text-slate-500 bg-slate-100 rounded-2xl hover:bg-slate-200 transition-colors"
      >
        관리자 콘텐츠 관리
      </button>
